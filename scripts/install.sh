...
prompt "This script will install glistenlau/nvimdots to:"
echo "${DEST_DIR}"
...
if [[ "$USE_SSH" -eq "1" ]]; then
	if is_latest; then
		execute "git" "clone" "-b" "main" "${CLONE_ATTR[@]}" "git@github.com:glistenlau/nvimdots.git" "${DEST_DIR}"
	else
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
		prompt "Automatically redirecting you to legacy version..."
		execute "git" "clone" "-b" "0.7" "${CLONE_ATTR[@]}" "git@github.com:glistenlau/nvimdots.git" "${DEST_DIR}"
	fi
else
	if is_latest; then
		execute "git" "clone" "-b" "main" "${CLONE_ATTR[@]}" "https://github.com/glistenlau/nvimdots.git" "${DEST_DIR}"
	else
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
		prompt "Automatically redirecting you to legacy version..."
		execute "git" "clone" "-b" "0.7" "${CLONE_ATTR[@]}" "https://github.com/glistenlau/nvimdots.git" "${DEST_DIR}"
	fi
fi
...
- Project Homepage:
    ${tty_underline}https://github.com/glistenlau/nvimdots${tty_reset}
- Further documentation (including executables you ${tty_bold}must${tty_reset} install for full functionality):
    ${tty_underline}https://github.com/glistenlau/nvimdots/wiki/Prerequisites${tty_reset}
EOS
